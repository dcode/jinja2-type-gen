...
def test_analyzer_memoryview_in_extends_include():
    ...
    types = analyzer.analyze_source("fake source") # doesn't matter, we'll process templates_to_process
...
def test_analyzer_exception_in_extends_include_branch():
    ...
    types = analyzer.analyze_source("{% extends 'missing.j2' %}{% include 'missing2.j2' %}")
    assert len(types) == 0
